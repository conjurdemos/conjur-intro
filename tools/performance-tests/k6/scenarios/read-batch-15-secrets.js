...
const readfifteenSecretsBatchTrend = new Trend('http_req_duration_get_fifteen_secrets_batch', true);
const readfifteenSecretsBatchFailRate = new Rate('http_req_failed_get_fifteen_secrets_batch');
...
export default function () {
  ...
  readfifteenSecretsBatchTrend.add(res.timings.duration);
  readfifteenSecretsBatchFailRate.add(res.status !== 200);
...
export function handleSummary(data) {
  return {
    "./tools/performance-tests/k6/reports/read-batch-15-secrets-summary.html": htmlReport(data, {title: "Read Batch 15 Secrets " + new Date().toISOString().slice(0, 16).replace('T', ' ')}),
