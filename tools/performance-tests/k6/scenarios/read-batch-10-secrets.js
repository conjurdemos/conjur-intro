...
const readtenSecretsBatchTrend = new Trend('http_req_duration_get_ten_secrets_batch', true);
const readtenSecretsBatchFailRate = new Rate('http_req_failed_get_ten_secrets_batch');
...
export default function () {
  ...
  readtenSecretsBatchFailRate.add(res.status !== 200);
...
export function handleSummary(data) {
  return {
    "./tools/performance-tests/k6/reports/read-batch-10-secrets-summary.html": htmlReport(data, {title: "Read Batch 10 Secrets " + new Date().toISOString().slice(0, 16).replace('T', ' ')}),
