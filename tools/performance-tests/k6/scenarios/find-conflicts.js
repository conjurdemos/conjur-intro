...
import {createConflictsPolicy} from "../modules/lib.js";
...
export default function () {
  ...
  const lobsPolicyRes = conjurApi.loadPolicy(
    ...
    env,
    policyId,
    lib.createConflictsPolicy(identifier)
  );

  console.log('lobsPolicyRes status:', lobsPolicyRes.status);

  console.log('');

  createPolicyTrend.add(lobsPolicyRes.timings.duration);
  createPolicyFailRate.add(lobsPolicyRes.status !== 201);

  check(lobsPolicyRes, {
    "status is 201": (r) => r.status === 201,
    "status is not 500": (r) => r.status !== 500,
    "status is not 409": (r) => r.status !== 409
  });
}
