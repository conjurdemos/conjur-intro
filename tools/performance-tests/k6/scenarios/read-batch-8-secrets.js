...
const readeightSecretsBatchTrend = new Trend('http_req_duration_get_eight_secrets_batch', true);
const readeightSecretsBatchFailRate = new Rate('http_req_failed_get_eight_secrets_batch');
...
export default function () {
  ...
  readeightSecretsBatchTrend.add(res.timings.duration);
  readeightSecretsBatchFailRate.add(res.status !== 200);
...
export function handleSummary(data) {
  return {
    "./tools/performance-tests/k6/reports/read-batch-8-secrets-summary.html": htmlReport(data, {title: "Read Batch 8 Secrets " + new Date().toISOString().slice(0, 16).replace('T', ' ')}),
