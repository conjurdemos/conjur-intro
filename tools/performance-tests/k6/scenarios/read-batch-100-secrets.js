...
const readhundredSecretsBatchTrend = new Trend('http_req_duration_get_hundred_secrets_batch', true);
const readhundredSecretsBatchFailRate = new Rate('http_req_failed_get_hundred_secrets_batch');
...
export default function () {
  ...
  readhundredSecretsBatchTrend.add(res.timings.duration);
  readhundredSecretsBatchFailRate.add(res.status !== 200);
...
export function handleSummary(data) {
  return {
    "./tools/performance-tests/k6/reports/read-batch-100-secrets-summary.html": htmlReport(data, {title: "Read Batch 100 Secrets " + new Date().toISOString().slice(0, 16).replace('T', ' ')}),
